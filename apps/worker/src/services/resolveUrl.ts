 ...
 * - IP リテラルをプライベート/ループバック/リンクローカル/予約済みレンジで直接ブロック
 * - 各リダイレクト先も毎回検査する
 * - URL に認証情報が含まれる場合は拒否
 *
 * ## Node.js と Cloudflare Workers の SSRF 防御レベルの差
 *
 * Node.js 環境:
 *   - `node:dns/promises` で実際に DNS 解決し、解決後の IP アドレスを検査する。
 *   - `internal.example.com → 10.0.0.1` のような DNS を使った内部到達をブロックできる。
 *
 * Cloudflare Workers 環境:
 *   - `node:dns/promises` が存在しないため DNS 解決後の IP 検査は行えない（スキップ）。
 *   - IP リテラル（`192.168.1.1`、`127.0.0.1` 等）はスキーム検証時に直接ブロックする。
 *   - URL・hostname・各リダイレクト先のスキーム/認証情報/形式は検査する。
 *   - ホスト名の DNS 解決結果に対する IP 検査ができないため DNS rebinding を完全には防げない。
 *   - Workers ランタイムの fetch サンドボックスに DNS 解決を委ねる設計だが、
 *     それが内部アドレスへのアクセスを防ぐかはランタイムの実装に依存する。
 *   - この実装を「完全な SSRF 防御」と見なさないこと。
 *
 * 共通:
 *   - プライベート IP の直接指定（IP リテラル）は Node.js・Workers 両環境で必ずブロックする。
 *   - 各リダイレクト先も毎回 validateUrl() で検査する。
 */
